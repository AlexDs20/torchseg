...
def get_stats(
    ...
    Args:
        ...
        ignore_index (Optional[int]): Label to ignore on for metric computation.
            Works for ``'binary'`` and ``'multilabel'`` modes.  Defaults to None.
    ...
    if mode == "multiclass" and ignore_index is not None and 0 <= ignore_index <= num_classes - 1:
        raise ValueError(
    ...
    if mode == "multiclass":
        ...
        tp, fp, fn, tn = _get_stats_multilabel(output, target, ignore_index)
...
def _get_stats_multilabel(
    ...
    tp = torch.zeros(batch_size, num_classes, dtype=torch.long)
    tn = torch.zeros(batch_size, num_classes, dtype=torch.long)
    fp = torch.zeros(batch_size, num_classes, dtype=torch.long)
    fn = torch.zeros(batch_size, num_classes, dtype=torch.long)

    for i, (pred_img, target_img) in enumerate(zip(output, target)):
